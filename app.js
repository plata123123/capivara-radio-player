...
document.addEventListener('click',e=>{
 ...
 if(t.includes('mandar pra fila')){
  setTimeout(capResetCreationAfterQueue,180)
 }
...
const capAudioObserver=new MutationObserver(()=>{
 const playable=[...document.querySelectorAll('audio')].some(
  a=>a.src||a.querySelector('source[src]')
 );
 if(playable&&!capPendingDecision)capMarkPending()
});

capAudioObserver.observe(document.documentElement,{
 subtree:true,
 childList:true,
 attributes:true,
 attributeFilter:['src']
...
function capResetCreationAfterQueue(){
 ...
 .filter(b=>
  (b.textContent||'').toLowerCase().includes('áudio pronto')||
  (b.textContent||'').toLowerCase().includes('audio pronto')
 )
 .forEach(b=>{
 ...
 document.querySelectorAll(
  '.product.active,.product.selected,.product-btn.active,.product-btn.selected'
 ).forEach(el=>{
...
function capSinglePhraseUI(){
 const genBtns=[...document.querySelectorAll('button')].filter(b=>{
  const t=(b.textContent||'').toLowerCase();
  return t.includes('gerar áudio')||
         t.includes('gerar audio')||
         t.includes('áudio pronto')||
         t.includes('audio pronto')
 });

 if(!genBtns.length)return;

 const cards=genBtns.map(
  b=>b.closest('.card,.option,.suggestion,.result,.ad-option')||b.parentElement
 ).filter(Boolean);
 ...
 if(first&&!first.querySelector('.cap-regenerate')){
  const b=document.createElement('button');
  b.type='button';
  b.className='btn secondary cap-regenerate';
...
const capSingleObserver=new MutationObserver(
 ()=>setTimeout(capSinglePhraseUI,20)
);

capSingleObserver.observe(document.documentElement,{
 subtree:true,
 childList:true
});
...
capBedCursorV36=0,
capJingleCursorV36={open:0,close:0};

function capModeCountV16(){
 const el=document.getElementById('adsPerBlock');
 const c=(()=>{
  try{return JSON.parse(localStorage.getItem('capivara_admin_settings')||'{}')}
  catch(e){return {}}
 })();
...
function capActiveAdsV16(){
 ...
 try{
  arr=JSON.parse(localStorage.getItem(capClientKey('cap_ads'))||'[]')
 }catch(e){
  arr=[]
 }
...
async function capPlayUrlV16(url,volume=1){
 ...
 return await new Promise(resolve=>{
  ...
  window.capCurrentSpokenAudio=a;
...
async function capPlayAdV16(ad){
 ...
 const url=ad.audioUrl||ad.url||ad.audio||ad.src;
 ...
 if(url){
  return await capPlayUrlV16(
   ...
   Math.max(
    ...
    Math.min(
     1,
     Number(document.getElementById('adVol')?.value||100)/100
    )
   )
  )
 }
 ...
 if(ad.audioKey){
  try{
   const blob=await capGetAdBlob(ad.audioKey);

   if(blob){
    const u=URL.createObjectURL(blob),
    ok=await capPlayUrlV16(
     u,
     Math.max(
      0,
      Math.min(
...
function capOpenDbV36(name,version,storeName){
 return new Promise((ok,no)=>{
  const r=indexedDB.open(name,version,storeName);
  r.onsuccess=()=>ok(r.result);
  r.onerror=()=>no(r.error)
 })
}
...
async function capGetDbBlobV36(dbName,version,storeName,key){
 try{
  const db=await capOpenDbV36(dbName,version,storeName);
...
async function capOptionalJingleV16(kind){
 ...
 try{
  ...
  if(ready.length){
   const ck=kind==='opening'?'open':'close',
   v=ready[capJingleCursorV36[ck]++%ready.length];

   const blob=await capGetDbBlobV36(
    'CapivaraRadioAudio',
    1,
    'audios',
    v.audioKey
  ...
  const legacy=JSON.parse(
   localStorage.getItem('capivara_radio_jingles')||'{}'
  );

  const j=legacy[kind];

  if(j&&!j.paused&&j.active!==false){
   return await capPlayUrlV16(j.audioUrl||j.url||j.src,1)
  }

 }catch(e){
  console.error('Vinheta:',e)
 }
...
async function capStartBedV36(){
 try{
  const bg=JSON.parse(
   localStorage.getItem('capivara_backgrounds_v9')||'[]'
  );
  ...
  a.volume=Math.max(
   ...
   Math.min(
    1,
    (+(document.getElementById('bedVol')?.value||6))/100
   )
  );
...
function capStopBedV36(a){
 ...
 try{
  ...
  if(window.capCurrentBedAudio===a){
   window.capCurrentBedAudio=null
  }

  if(a._capUrl)URL.revokeObjectURL(a._capUrl)
...
async function capRadioAfterMusicV16(){
 ...
 const qty=Math.min(capModeCountV16(),active.length),
 block=[];

 for(let i=0;i<qty;i++){
  block.push(active[(capAdCursorV16+i)%active.length])
 }
...
function capResetCreateV20(){
 ...
 ['1','2'].forEach(n=>{
  if(pendingAudio[n]?.url){
   try{
    URL.revokeObjectURL(pendingAudio[n].url)
   }catch{}
  }
...
async function capCreateOneClickV20(){
 ...
 try{
  ...
  if(todayCreated()>=capLimits().daily){
   throw new Error('O limite diário de anúncios foi atingido.')
  }

  if(weekCreated()>=capLimits().weekly){
   throw new Error('O limite semanal de anúncios foi atingido.')
  }

  if(p.top&&topCreatedToday()>=capLimits().top){
   throw new Error('O limite diário de TOP foi atingido.')
  }
  ...
  localStorage.setItem(
   capClientKey('cap_voice_queued_count'),
   String(
    +(localStorage.getItem(capClientKey('cap_voice_queued_count'))||0)+1
   )
  );
...
function capSetStepModeV20(on){
 ...
 if(b){
  b.textContent=on?'⚡ VOLTAR PARA 1 CLIQUE':'⚙ FAZER PASSO A PASSO'
 }
}
...
window.addEventListener('DOMContentLoaded',()=>{
 ...
 capSetStepModeV20(
  localStorage.getItem('cap_step_mode')==='1'
 );

 if(b){
  b.onclick=()=>capSetStepModeV20(
   !document.body.classList.contains('step-mode-v20')
  )
 }
...
function capDiscardV21(){
 if(pendingAudio[1]?.url){
  try{
   URL.revokeObjectURL(pendingAudio[1].url)
  }catch{}
 }
...
async function capGenerateAndQueueV22(){
 ...
 try{
  ...
  if(!text){
   throw new Error('O texto do anúncio está vazio.')
  }
  ...
  if(pa.top&&topCreatedToday()>=capLimits().top){
   throw new Error('O limite diário de TOP foi atingido.')
  }
...
window.addEventListener('DOMContentLoaded',()=>{
 ...
 const clean=()=>document.querySelectorAll('button').forEach(x=>{
  if(/gerar outra/i.test((x.textContent||'').trim())){
   x.remove()
  }
 });
...
function capV25SetMainButtons(show){
 ...
 if(create){
  create.style.display=show?'':'none'
 }

 document.querySelectorAll('button').forEach(b=>{
  if(/top do dia/i.test((b.textContent||'').trim())){
...
(function(){
 ...
 function applyMusic(){
  const e=document.getElementById('musicVol');
  if(!e)return;
 ...
 function applyBed(){
  const e=document.getElementById('bedVol');
  if(!e)return;
  ...
  if(window.capCurrentBedAudio){
   window.capCurrentBedAudio.volume=v
  }
 }

 function bind(){
  const music=document.getElementById('musicVol'),
  bed=document.getElementById('bedVol');

  if(music&&!music.dataset.v39){
   music.dataset.v39='1';

   const sv=localStorage.getItem('cap_musicVol');
  ...
  if(bed&&!bed.dataset.v39){
   ...
   if(sv!==null){
    ...
    bed.value='6';
    localStorage.setItem('cap_bedVol','6')
   }
...
(function(){
 ...
 function update(){
  ...
  if(w){
   w.style.display=manualText()?'block':'none'
  }
 }
 ...
 async function run(){
  ...
  if(typeof capGenerateQueueV24==='function'){
   ...
   await capGenerateAudioDirectV201(t);
 ...
 try{
  ...
  if(Array.isArray(s.ads)){
   ads=s.ads.filter(
    a=>
     !a.clientCode||
     String(a.clientCode)===String(store.code)
   );
  ...
  if(Number.isFinite(+s.voiceTurn)){
   voiceTurn=+s.voiceTurn;

   localStorage.setItem(
    capClientKey('cap_voice'),
    String(voiceTurn)
   );
  }
  ...
  if(s.adsPerBlock){
   localStorage.setItem(
    'cap_ads_per_block_'+store.code,
    String(s.adsPerBlock)
   );
  }
